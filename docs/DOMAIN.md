...
**REGRA DEFINIDA (RN29 — Sprint 38, ampliada Sprint 44, auditada Sprint 45):** todo paciente possui uma **origem** (`origem_paciente`: `regular` | `esporadico`) — atributo do **PACIENTE**. Todo registro de vale possui um **tipo** (`tipo_liberacao`: `continua` | `avulsa`) — atributo da **LIBERAÇÃO**. São conceitos distintos e ortogonais, auditados em domínio (`validarLiberacao({ origemPaciente, tipo })`), action/service (`criarLiberacaoAction` repassa `paciente.origem`), trigger (`fn_liberacoes_before` linha `p.origem = esporadico AND tipo != avulsa → RN29`), RLS e UI (`liberacao-form` bloqueia contínua para esporádico). **Nomenclatura oficial:** paciente "Regular/Esporádico", liberação "Contínua/Avulsa" — evitar "liberação esporádica" como termo técnico; preferir "liberação avulsa para paciente esporádico". Paciente **esporádico** pode ser cadastrado por **qualquer dos três perfis** quando necessário para liberação avulsa, e **somente pode receber liberação avulsa** (nunca contínua). Pacientes regulares são cadastrados por Gestor ou Autorizador; Recepcionista **não cadastra regular independente** — apenas **localiza/reutiliza** o cadastro existente. A reutilização é obrigatória: um mesmo paciente esporádico (`ex.: João, origem=esporadico → liberações avulsas A,B,C`) pode possuir **várias liberações avulsas ao longo do tempo** (`1 paciente → N liberações → N retiradas`), não devendo ser recriado a cada atendimento. **Matriz de compatibilidade auditada Sprint 45:** `regular + continua = válido`, `regular + avulsa = válido`, `esporadico + avulsa = válido`, `esporadico + continua = INVÁLIDO (bloqueado em todas as camadas)`. Dados existentes: 0 combinações inválidas em DEV e PROD (verificado 2026-09-02). A regra RN29 é garantida no banco (trigger `fn_liberacoes_before`), não apenas na UI.
