...
\echo '============================================================'
\echo 'VALIDACAO SUPABASE SPRINT 09 — SOMENTE LEITURA'
\echo '============================================================'
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 1. TABELAS ESPERADAS EM public ==='
...
\echo ''
\echo '=== 1b. TABELAS BASE (relkind = r) ==='
select tablename
...
\echo ''
\echo '=== 5b. UNIQUE (índices, inclui parciais ex: cpf where not null) ==='
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 6. CHECK CONSTRAINTS (definição real) ==='
...
\echo ''
\echo '=== 11b. VIEWS — definição real (inclui security_invoker/barrier) ==='
select v.viewname,
       coalesce((
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 12. ROW LEVEL SECURITY (relrowsecurity = on) ==='
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 14. PRIVILÉGIOS DE TABELAS/VIEWS (grantee × privilégio) ==='
select grantee,
       table_name,
...
\echo ''
\echo '=== 14b. PRIVILÉGIOS DE COLUNA (ex: cpf) ==='
select grantee,
       table_name,
       column_name,
       privilege_type
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 15. EXECUTE EM FUNÇÕES (grants) ==='
select grantee,
       routine_name,
       string_agg(privilege_type, ', ' order by privilege_type) as privilegios
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 16. auth.users sem registro em public.usuarios ==='
...
\echo ''
\echo '=== 16b. VÍNCULOS EXISTENTES (usuarios × auth.users) ==='
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 17. SEGURANÇA DO CPF ==='
\echo '--- 17.1 Quem tem SELECT direto na coluna cpf (esperado: ninguém além do owner) ---'
...
\echo '--- 17.2 v_pacientes NÃO deve selecionar cpf (definição abaixo) ---'
select pg_get_viewdef(('public.v_pacientes')::regclass, true) as definicao_v_pacientes;

\echo '--- 17.3 função pacientes_com_cpf(): deve exigir gestor + ativo (definição abaixo) ---'
...
\echo ''
\echo '=== 18. AUDITORIA ==='
\echo '--- 18.1 função fn_auditoria (gera logs a partir da sessão) ---'
select pg_get_functiondef(p.oid) as definicao_fn_auditoria
from pg_catalog.pg_proc p
join pg_catalog.pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public' and p.proname = 'fn_auditoria';

\echo '--- 18.2 função fn_auditoria_imutavel (append-only) ---'
...
\echo '--- 18.3 triggers de auditoria (devem existir em pacientes/usuarios/liberacoes/retiradas) ---'
...
\echo '--- 18.4 append-only: quem pode INSERT/UPDATE/DELETE em auditoria_logs ---'
select grantee, privilege_type
from information_schema.role_table_grants
where table_schema = 'public'
...
-- ----------------------------------------------------------------------------
\echo ''
\echo '=== 19. INTEGRIDADE (regras de domínio) ==='
\echo '--- 19.1 quantidade 1/2/4/8 (CHECK em liberacoes) ---'
...
\echo '--- 19.2 contínua 1/3/6 meses; avulsa sem período (CHECK periodo em liberacoes) ---'
select conname, pg_get_constraintdef(oid) as definicao
from pg_catalog.pg_constraint
where conrelid = 'public.liberacoes'::regclass
  and contype = 'c'
...
\echo '--- 19.3 data_fim > data_inicio (CHECK em liberacoes) ---'
select conname, pg_get_constraintdef(oid) as definicao
from pg_catalog.pg_constraint
where conrelid = 'public.liberacoes'::regclass
  and contype = 'c'
...
\echo '--- 19.4 retiradas.quantidade > 0 (CHECK em retiradas) ---'
select conname, pg_get_constraintdef(oid) as definicao
from pg_catalog.pg_constraint
...
\echo '--- 19.5 validações em trigger (autorizador ativo RN02/RN27; paciente ativo RN01;' 
\echo '---     retirada do paciente correto RN24; validade; quantidade restante RN14/RN22) ---'
select p.proname as funcao
from pg_catalog.pg_proc p
join pg_catalog.pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
...
\echo ''
\echo '=== FIM DA VALIDAÇÃO (somente leitura — nenhuma alteração executada) ==='
